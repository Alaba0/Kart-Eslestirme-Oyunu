...
public class PictureManager : MonoBehaviour
{
    ...
    private Vector2 _offset = new Vector2(x:1.5f, 1.52f);
    private Vector2 _offsetFor15Pairs = new Vector2(x: 1.08f, y: 1.22f);
    private Vector2 _offsetFor20Pairs = new Vector2(x: 1.08f, y: 1.0f);
    private Vector3 _newScaleDown = new Vector3(x: 0.9f, y: 0.9f, z: 0.001f);
    ...
    private int _secondRevealedPic2;
    private int _revealedPicNumber = 0;
    private int _picDestroy1;
    private int _picDestroy2;
    ...
    void Start()
    {
        ...
        if(GameSettings.Instance.GetPairNumber() == GameSettings.EPairNumber.E10Pair)
        {
           ...
           SpwanPictureMesh(rows:4,columns: 5,Pos: StartPosition, _offset,scaledown: false);
           MovePicture(rows:4,columns: 5,pos: StartPosition, _offset);
        }
        else if (GameSettings.Instance.GetPairNumber() == GameSettings.EPairNumber.E15Pair)
        {
            CurrentGameState = GameState.MovingOnPositions;
            SpwanPictureMesh(rows:5,columns: 6,Pos: StartPosition, _offset,scaledown: false);
            MovePicture(rows:5,columns: 6,pos:StartPosition, _offsetFor15Pairs);
        }
        else if (GameSettings.Instance.GetPairNumber() == GameSettings.EPairNumber.E20Pair)
        {
            CurrentGameState = GameState.MovingOnPositions;
            SpwanPictureMesh(rows:5,columns: 8,Pos: StartPosition, _offset,scaledown: true);
            MovePicture(rows:5,columns: 8,pos: StartPosition, _offsetFor20Pairs);
        }
    }
    ...
    public void CheckPicture()
    {
        ...
        {
            if (PictureList[_firstRevealedPic].GetIndex() == PictureList[_secondRevealedPic2].GetIndex() && _firstRevealedPic != _secondRevealedPic2)
            {
                CurrentGameState = GameState.DeletingPuzzles;
                _picDestroy1 = _firstRevealedPic;
                _picDestroy2 = _secondRevealedPic2;
            }
            else
            {
    ...
    {
        PuzzleRevealedNumber = RevealedState.NoRevealed;
        PictureList[_picDestroy1].Deactivate();
        PictureList[_picDestroy2].Deactivate();
        _revealedPicNumber = 0;
        _removedPairs++;
        CurrentGameState = GameState.NoAction;
        CurrentPuzzleState = PuzzleState.CanRotate;
    ...
    private IEnumerator FlipBack()
    {
        ...
        PictureList[_secondRevealedPic2].FlipBack();
    ...
    void Update()
    {
        ...
        if(CurrentGameState == GameState.FlipBack)
        {
            ...
            {
                StartCoroutine(routine: FlipBack());
            }
        }
        ...
        if(CurrentGameState == GameState.GameEnd)
        {
            if (PictureList[_firstRevealedPic].gameObject.activeSelf == false &&
                PictureList[_secondRevealedPic2].gameObject.activeSelf == false &&
    ...
    private void ShowEndGameInformation()
    {
        ...
        var minutes = Mathf.Floor(f:timer / 60);
        var seconds = Mathf.RoundToInt(f:timer % 60);
        var newText = minutes.ToString(format: "00") + ":" + seconds.ToString(format: "00");
    ...
    private void SpwanPictureMesh(int rows, int columns, Vector2 Pos, Vector2 offset, bool scaledown)
    {
        for (int col = 0; col < columns; col++)
        {
            for (int row = 0; row < rows; row++)
            {
                ...
                tempPicture.transform.Rotate(0, 0, 180);
